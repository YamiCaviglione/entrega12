...
import { useFaucetUsers } from '../hooks/useFaucetContract'
import { FormatUtils, UsersPagination } from '../types'

interface UserListProps {
  itemsPerPage?: number
}

export function UserList({ itemsPerPage = 10 }: UserListProps) {
  const { users, error, isLoading, refetch } = useFaucetUsers()
  const [currentPage, setCurrentPage] = useState(1)

  // Calcular paginación
  const pagination: UsersPagination = useMemo(() => {
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="bg-gray-50 p-4 rounded-lg mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          ...
          <div>
            <p className="text-2xl font-bold text-green-600">{pagination.totalPages}</p>
            <p className="text-sm text-gray-600">Páginas</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-purple-600">{pagination.currentPage}</p>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {pagination.users.map((address, index) => {
      ...
      {pagination.totalUsers > 0 && (
        <div className="mt-4 text-center text-sm text-gray-500">
          Mostrando {Math.min(itemsPerPage, pagination.totalUsers - (pagination.currentPage - 1) * itemsPerPage)} de {pagination.totalUsers} usuarios
        </div>
      )}
    </div>
  )
